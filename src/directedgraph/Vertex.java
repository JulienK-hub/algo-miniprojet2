...
class Vertex<T> {
     ...
     *          the edge cost
     */
    public void addOutgoingEdge(Vertex<T> to, int cost, Color color) {
        Edge<T> out = new Edge<T>(this, to, cost, color);
     ...
     *          the edge cost
     */
    public void addIncomingEdge(Vertex<T> from, int cost, Color color) {
        Edge<T> out = new Edge<T>(this, from, cost, color);
    ...
    public String toString() {
        ...
        if (name.indexOf('R') > -1) tmp += ConsoleColors.RED;
        if (name.indexOf('B') > -1) tmp += ConsoleColors.BLUE;
        tmp += name;
        ...
        for (int i = 0; i < incomingEdges.size(); i++) {
            ...
            tmp += ConsoleColors.RESET;
            if(e.getColor() == Color.RED) tmp += ConsoleColors.RED;
            else tmp += ConsoleColors.BLUE;
            tmp += " <-- ";
            tmp += ConsoleColors.RESET;
        }
